...
$("#create-form").addEventListener("submit", async (event) => {
  ...
  const materials = String(form.get("materials") || "").split(/\n+/).map((value) => value.trim()).filter(Boolean); const body = { name: form.get("name"), topic: form.get("topic"), platforms, video_materials: materials, presentation_mode: form.get("presentation_mode") || "narration", lip_sync_mode: form.get("lip_sync_mode") || "auto", interval_minutes: Number(form.get("interval_minutes")), enabled: form.get("enabled") === "on" };
