...
const lipSyncModeNames = { auto: "自动选择", fast: "本机快速", quality: "高质量" };
const contentTypeNames = { standard: "通用短视频", ai_comic: "AI 动态漫剧" };
const templates = {
  "ai-tools": { name: "每日 AI 工具观察", topic: "面向普通创作者，解读值得关注的本地 AI 工具、自动化实践和真实使用体验。", contentType: "standard", interval: "1440", platforms: ["douyin", "xiaohongshu", "bilibili", "youtube"] },
  knowledge: { name: "每周知识解读", topic: "把一个复杂知识点讲清楚，强调事实、结构、例子和可执行结论。", contentType: "standard", interval: "10080", platforms: ["bilibili", "youtube"] },
  story: { name: "都市异能漫剧", topic: "创作一部原创都市异能竖屏漫剧：两个主要角色，在日常场景中发现异常线索，前3秒进入冲突，中段反转，结尾留下下一集悬念。画风为精致二维国漫，角色外观和服装必须连续一致。", contentType: "ai_comic", interval: "1440", platforms: ["douyin", "xiaohongshu"] },
};
...
function currentOperation(task) {
  ...
  const shots = task.content_plan?.shots || [];
  const complete = shots.filter((shot) => shot.status === "complete").length;
  const lipStageNames = { queued: "等待口型推理资源", preparing: "准备音视频", inference: "口型引擎正在推理", quality_check: "检查口型与正脸质量", complete: "口型同步完成", failed: "口型同步失败" };
  ...
  const messages = {
    ...
    lip_syncing: `${lipStage}${lipElapsed ? `，已运行 ${formatElapsedSeconds(lipElapsed)}` : ""}。当前使用${lipSyncModeNames[task.lip_sync_mode] || "自动选择"}，完成后将自动进入质量门禁。`,
...
function renderShotList(task) {
  ...
  return `<div class="shot-list">${shots.map((shot) => {
    const requested = shot.presentation_mode || "narration"; const effective = shot.effective_presentation_mode || requested; const fallback = shot.lip_sync_fallback_reason;
    const quality = shot.lip_sync_score != null ? `同步 ${Math.round(shot.lip_sync_score * 100)} · 正脸 ${Math.round((shot.face_coverage || 0) * 100)}` : ""; const lipProvider = shot.lip_sync_provider ? ` · ${shot.lip_sync_provider}` : ""; const isComic = task.content_type === "ai_comic";
    const performance = isComic ? `<div class="shot-mode-row"><b>${escapeHtml(shot.shot_type || "COMIC")}</b><span>${escapeHtml(shot.scene_id || "未命名场景")} · ${escapeHtml(shot.camera_motion || "hold")} · ${escapeHtml(shot.emotion || "中性")}</span></div>${shot.dialogue ? `<div class="comic-dialogue"><span>${escapeHtml(shot.speaker || "旁白")}</span><p>${escapeHtml(shot.dialogue)}</p></div>` : ""}` : `<div class="shot-mode-row"><b>${escapeHtml(presentationNames[requested] || requested)}</b><span>${effective !== requested ? `实际：${escapeHtml(presentationNames[effective] || effective)}` : `${quality || "按计划执行"}${escapeHtml(lipProvider)}`}</span></div>`;
    const preview = shot.media_path ? `<details class="shot-preview"><summary>预览分镜素材</summary>${isComic && shot.keyframe_path ? `<img src="/tasks/${encodeURIComponent(task.id)}/shots/${encodeURIComponent(shot.id)}/keyframe" alt="分镜 ${shot.order} 关键帧" loading="lazy" />` : ""}<video controls preload="none" playsinline src="/tasks/${encodeURIComponent(task.id)}/shots/${encodeURIComponent(shot.id)}/preview"></video></details><div class="shot-path"><code>${escapeHtml(shot.media_path)}</code><button type="button" data-copy-value="${escapeHtml(shot.media_path)}">复制</button></div>` : "";
    return `<article class="shot-card ${requested === "talking_head" ? "is-talking" : ""} ${isComic ? "is-comic" : ""}"><div class="shot-index"><span>${isComic ? "PANEL" : "SHOT"}</span><b>${String(shot.order).padStart(2, "0")}</b></div><div class="shot-body"><div class="shot-head">${badge(shot.status)}<span>${shot.duration_seconds} 秒 · ${isComic ? "关键帧动画" : shot.kind === "image" ? "图片" : "视频"}</span><span>${escapeHtml(mediaProviderName(shot.provider))}</span></div>${performance}<h4>${escapeHtml(shot.narration)}</h4><p>${escapeHtml(shot.visual_prompt)}</p>${fallback ? `<div class="lip-fallback"><b>已安全降级</b><span>${escapeHtml(fallback)}</span></div>` : ""}${shot.audio_path ? `<div class="shot-audio">独立驱动音频 · <code>${escapeHtml(shot.audio_path)}</code></div>` : ""}${shot.negative_prompt ? `<details><summary>负面提示词</summary><p>${escapeHtml(shot.negative_prompt)}</p></details>` : ""}${shot.error || shot.lip_sync_error ? `<div class="inline-error">${escapeHtml(shot.error || shot.lip_sync_error)}</div>` : ""}${preview}</div></article>`;
  }).join("")}</div>`;
}
...
function renderComicBrief(task) {
  if (task.content_type !== "ai_comic" || !task.content_plan) return "";
  const plan = task.content_plan; const characters = plan.characters || [];
  return `<div class="comic-brief"><div><span>本集故事</span><p>${escapeHtml(plan.story_summary || "等待故事梗概")}</p><small>${escapeHtml(plan.episode_goal || "")}</small></div><div class="comic-cast"><span>角色阵容</span>${characters.length ? characters.map((character) => `<article><b>${escapeHtml(character.name)}</b><em>${escapeHtml(character.role || "主要角色")}</em><p>${escapeHtml(character.personality || character.appearance_prompt)}</p><small>${escapeHtml(character.voice || "Vivian")} · ${escapeHtml(character.outfit_prompt || "固定造型")}</small></article>`).join("") : "<p>角色设定尚未生成。</p>"}</div></div>`;
}
...
function openTaskDetail(taskId) {
  ...
  const contentBrief = $("#detail-content-package .brief-grid");
  if (contentBrief && task.content_type === "ai_comic") contentBrief.insertAdjacentHTML("afterend", renderComicBrief(task));
...
function applyTemplate(key) {
  const template = templates[key]; if (!template) return; const form = $("#create-form"); form.elements.name.value = template.name; form.elements.topic.value = template.topic; form.elements.content_type.value = template.contentType || "standard"; form.elements.interval_minutes.value = template.interval; form.querySelectorAll('[name="platforms"]').forEach((input) => { input.checked = template.platforms.includes(input.value); }); syncContentTypeControls();
}

function syncContentTypeControls() {
  const form = $("#create-form"); const isComic = form.elements.content_type.value === "ai_comic";
  $("#presentation-fieldset").hidden = isComic; $("#lip-sync-fieldset").hidden = isComic; $("#comic-mode-note").hidden = !isComic;
  $("#generation-step-intro").textContent = isComic ? "漫剧采用角色和对白优先的关键帧生产线，避免视频模型逐帧重画导致角色变形。" : "选择人物如何出镜。唇形模型未安装时，讲话镜头会明确降级为旁白，不会生成错误口型。";
  $("#estimated-flow").textContent = isComic ? "角色设定 → 对白分镜 → 动漫关键帧 → 二维运镜 → 配音字幕 → 审核" : "策划 → 素材 → 分镜配音 → 口型门禁 → 补帧 → 合成 → 审核";
  $("#estimated-flow-note").textContent = isComic ? "V1 使用稳定关键帧和 48 FPS 镜头运动；不启用真人嘴型融合。" : "自动模式会优先选择当前可用的最高质量口型引擎，并记录实际 Provider。";
  if (isComic) { form.elements.presentation_mode.value = "narration"; form.elements.lip_sync_mode.value = "auto"; }
}

function renderPlanReview() { const form = new FormData($("#create-form")); const platforms = form.getAll("platforms").map((item) => platformNames[item] || item); const isComic = form.get("content_type") === "ai_comic"; $("#plan-review").innerHTML = `<span>确认计划</span><h3>${escapeHtml(form.get("name") || "未命名计划")}</h3><p>${escapeHtml(form.get("topic") || "尚未填写内容主题")}</p><div><b>${escapeHtml(contentTypeNames[form.get("content_type")] || contentTypeNames.standard)}</b>${platforms.map((item) => `<b>${escapeHtml(item)}</b>`).join("")}${isComic ? "" : `<b>${escapeHtml(presentationNames[form.get("presentation_mode")] || "旁白镜头")}</b><b>${escapeHtml(lipSyncModeNames[form.get("lip_sync_mode")] || "自动选择")}</b>`}<b>每 ${escapeHtml(form.get("interval_minutes"))} 分钟</b><b>${form.get("enabled") === "on" ? "定时开启" : "仅手动运行"}</b></div>`; }
...
function openPlanForm({ automation = null, template = "" } = {}) {
  ...
  if (automation) { form.elements.name.value = automation.name; form.elements.topic.value = automation.topic; form.elements.content_type.value = automation.content_type || "standard"; form.elements.interval_minutes.value = String(automation.interval_minutes); form.elements.materials.value = automation.video_materials.join("\n"); form.elements.presentation_mode.value = automation.presentation_mode || "narration"; form.elements.lip_sync_mode.value = automation.lip_sync_mode || "auto"; form.elements.enabled.checked = automation.enabled; form.querySelectorAll('[name="platforms"]').forEach((input) => { input.checked = automation.platforms.includes(input.value); }); }
  if (template) applyTemplate(template); syncContentTypeControls(); setWizardStep(1); $("#create-modal").showModal(); setTimeout(() => form.elements.name.focus(), 60);
}
...
async function handleAutomationAction(button) {
  ...
  if (action === "duplicate") { try { await api("/automations", { method: "POST", body: JSON.stringify({ name: `${automation.name}（副本）`, topic: automation.topic, platforms: automation.platforms, video_materials: automation.video_materials, content_type: automation.content_type || "standard", presentation_mode: automation.presentation_mode || "narration", lip_sync_mode: automation.lip_sync_mode || "auto", interval_minutes: automation.interval_minutes, enabled: false }) }); await loadData({ quiet: true }); showToast("计划已复制，默认保持暂停"); } catch (error) { showToast(error.message); } return; }
...
$("#create-form").addEventListener("submit", async (event) => {
  ...
  const materials = String(form.get("materials") || "").split(/\n+/).map((value) => value.trim()).filter(Boolean); const body = { name: form.get("name"), topic: form.get("topic"), platforms, video_materials: materials, content_type: form.get("content_type") || "standard", presentation_mode: form.get("presentation_mode") || "narration", lip_sync_mode: form.get("lip_sync_mode") || "auto", interval_minutes: Number(form.get("interval_minutes")), enabled: form.get("enabled") === "on" };
...
$$('[name="content_type"]').forEach((input) => input.addEventListener("change", syncContentTypeControls));
